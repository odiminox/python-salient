...
def find_sdl_attrs(prefix: str) -> Iterator[Tuple[str, Union[int, str, Any]]]:
    """Return names and values from `salient.lib`.

    `prefix` is used to filter out which names to copy.
    """
    from salient._libsalient import lib  # type: ignore
    ...
    with open("salient/constants.py", "w", encoding="utf-8") as f:
        ...
        for name in dir(lib):
            ...
            if name[:5] == "SALIENT_":
                ...
                all_names.append(name)
            elif name[:6] == "SALIENTK_":  # key name
        ...
        f.write(f"\n__all__ = [\n    {all_names_merged},\n]\n")
        update_module_all("salient/__init__.py", all_names_merged)
        update_module_all("salient/salientpy.py", all_names_merged)

    with open("salient/event_constants.py", "w", encoding="utf-8") as f:
        all_names = []
