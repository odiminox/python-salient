...
For example: ``salient.event.K_UP`` and ``salient.event.SCANCODE_A`` refer to
...
Example::
    ...
    KEY_COMMANDS = {
        salient.event.KeySym.UP: "move N",
        salient.event.KeySym.DOWN: "move S",
        salient.event.KeySym.LEFT: "move W",
        salient.event.KeySym.RIGHT: "move E",
    }

    context = salient.context.new()
    while True:
        console = context.new_console()
        context.present(console, integer_scaling=True)
        for event in salient.event.wait():
            context.convert_event(event)  # Adds tile coordinates to mouse events.
            if isinstance(event, salient.event.Quit):
                print(event)
                raise SystemExit()
            elif isinstance(event, salient.event.KeyDown):
            ...
            elif isinstance(event, salient.event.MouseButtonDown):
                print(event)  # Prints the mouse button constant names for this event.
            elif isinstance(event, salient.event.MouseMotion):
...
Example::
    ...
    while True:
        ...
        for event in salient.event.wait():
            context.convert_event(event)  # Adds tile coordinates to mouse events.
            match event:
                case salient.event.Quit():
                    raise SystemExit()
                case salient.event.KeyDown(sym) if sym in KEY_COMMANDS:
                    print(f"Command: {KEY_COMMANDS[sym]}")
                case salient.event.KeyDown(sym=sym, scancode=scancode, mod=mod, repeat=repeat):
                    print(f"KeyDown: {sym=}, {scancode=}, {mod=}, {repeat=}")
                case salient.event.MouseButtonDown(button=button, pixel=pixel, tile=tile):
                    print(f"MouseButtonDown: {button=}, {pixel=}, {tile=}")
                case salient.event.MouseMotion(pixel=pixel, pixel_motion=pixel_motion, tile=tile, tile_motion=tile_motion):
                    print(f"MouseMotion: {pixel=}, {pixel_motion=}, {tile=}, {tile_motion=}")
                case salient.event.Event() as event:
...
import salient.event_constants
import salient.sdl.joystick
from salient.event_constants import *  # noqa: F4
from salient.event_constants import KMOD_ALT, KMOD_CTRL, KMOD_GUI, KMOD_SHIFT
from salient.loader import ffi, lib
from salient.sdl.joystick import _HAT_DIRECTIONS
...
class _ConstantsWithPrefix(Mapping[int, str]):
    ...
    def __getitem__(self, key: int) -> str:
        return "salient.event." + self.constants[key]
...
def _pixel_to_tile(x: float, y: float) -> Optional[Tuple[float, float]]:
    ...
    if not lib.salient_ctx.engine:
        return None
    xy = ffi.new("double[2]", (x, y))
    lib.salient_sys_pixel_to_tile(xy, xy + 1)
    return xy[0], xy[1]
...
class Modifier(enum.IntFlag):
    ...
    Example::
        ...
        >>> mod = salient.event.Modifier(4098)
        >>> mod
        <Modifier.NUM|RSHIFT: 4098>
        >>> mod & salient.event.Modifier.SHIFT  # Check if any shift key is held.
        <Modifier.RSHIFT: 2>
        >>> mod & salient.event.Modifier.LSHIFT  # Check if left shift key is held.
        <Modifier.NONE: 0>
        >>> not mod & salient.event.Modifier.LSHIFT  # Check if left shift key is NOT held.
        True
        >>> mod & salient.event.Modifier.SHIFT and mod & salient.event.Modifier.CTRL  # Check if Shift+Control is held.
        <Modifier.NONE: 0>
...
# reverse tables are used to get the salient.event name from the value.
...
_REVERSE_MOD_TABLE = salient.event_constants._REVERSE_MOD_TABLE.copy()
...
class Quit(Event):
    ...
    def __repr__(self) -> str:
        return "salient.event.%s()" % (self.__class__.__name__,)
...
class KeyboardEvent(Event):
    ...
    Attributes:
        ...
        mod (Modifier): A bitmask of the currently held modifier keys.
            ...
            ``event.mod & salient.event.Modifier.SHIFT`` will evaluate to a true
            value.
    ...
    def __repr__(self) -> str:
        return "salient.event.%s(scancode=%r, sym=%r, mod=%s%s)" % (
    ...
    def __str__(self) -> str:
        return self.__repr__().replace("salient.event.", "")
...
class MouseState(Event):
    ...
    Attributes:
        ...
        state (int): A bitmask of which mouse buttons are currently held.
            ...
            * salient.event.BUTTON_LMASK
            * salient.event.BUTTON_MMASK
            * salient.event.BUTTON_RMASK
            * salient.event.BUTTON_X1MASK
            * salient.event.BUTTON_X2MASK
    ...
    def __repr__(self) -> str:
        return ("salient.event.%s(pixel=%r, tile=%r, state=%s)") % (
            self.__class__.__name__,
            tuple(self.pixel),
            tuple(self.tile),
...
class MouseMotion(MouseState):
    ...
    def __repr__(self) -> str:
        return ("salient.event.%s(pixel=%r, pixel_motion=%r, " "tile=%r, tile_motion=%r, state=%s)") % (
            self.__class__.__name__,
            tuple(self.pixel),
...
class MouseButtonEvent(MouseState):
    ...
    Attributes:
        ...
        button (int): Which mouse button.
            ...
            * salient.event.BUTTON_LEFT
            * salient.event.BUTTON_MIDDLE
            * salient.event.BUTTON_RIGHT
            * salient.event.BUTTON_X1
            * salient.event.BUTTON_X2
    """
    ...
    def __repr__(self) -> str:
        return "salient.event.%s(pixel=%r, tile=%r, button=%s)" % (
            self.__class__.__name__,
            tuple(self.pixel),
            tuple(self.tile),
...
class MouseWheel(Event):
    ...
    def __repr__(self) -> str:
        return "salient.event.%s(x=%i, y=%i%s)" % (
            self.__class__.__name__,
            self.x,
            self.y,
...
class TextInput(Event):
    ...
    def __repr__(self) -> str:
        return "salient.event.%s(text=%r)" % (self.__class__.__name__, self.text)
...
class WindowEvent(Event):
    ...
    def __repr__(self) -> str:
        return "salient.event.%s(type=%r)" % (self.__class__.__name__, self.type)
...
class WindowMoved(WindowEvent):
    ...
    def __repr__(self) -> str:
        return "salient.event.%s(type=%r, x=%r, y=%r)" % (
            self.__class__.__name__,
            self.type,
            self.x,
            self.y,
...
class WindowResized(WindowEvent):
    ...
    def __repr__(self) -> str:
        return "salient.event.%s(type=%r, width=%r, height=%r)" % (
            self.__class__.__name__,
            self.type,
            self.width,
            self.height,
...
class JoystickEvent(Event):
    ...
    @property
    def joystick(self) -> salient.sdl.joystick.Joystick:
        if self.type == "JOYDEVICEADDED":
            return salient.sdl.joystick.Joystick._open(self.which)
        return salient.sdl.joystick.Joystick._from_instance_id(self.which)

    def __repr__(self) -> str:
        return f"salient.event.{self.__class__.__name__}" f"(type={self.type!r}, which={self.which})"
...
class JoystickAxis(JoystickEvent):
    ...
    def __repr__(self) -> str:
        return (
            f"salient.event.{self.__class__.__name__}"
...
class JoystickHat(JoystickEvent):
    ...
    def __repr__(self) -> str:
        return (
            f"salient.event.{self.__class__.__name__}" f"(type={self.type!r}, which={self.which}, x={self.x}, y={self.y})"
        )
...
class JoystickButton(JoystickEvent):
    ...
    def __repr__(self) -> str:
        return f"salient.event.{self.__class__.__name__}" f"(type={self.type!r}, which={self.which}, button={self.button})"
...
class JoystickDevice(JoystickEvent):
    ...
    Example::
        ...
        joysticks: set[salient.sdl.joystick.Joystick] = {}
        for event in salient.event.get():
            match event:
                case salient.event.JoystickDevice(type="JOYDEVICEADDED", joystick=new_joystick):
                    joysticks.add(new_joystick)
                case salient.event.JoystickDevice(type="JOYDEVICEREMOVED", joystick=joystick):
...
class ControllerEvent(Event):
    ...
    @property
    def controller(self) -> salient.sdl.joystick.GameController:
        """The :any:`GameController` for this event."""
        if self.type == "CONTROLLERDEVICEADDED":
            return salient.sdl.joystick.GameController._open(self.which)
        return salient.sdl.joystick.GameController._from_instance_id(self.which)
...
class ControllerAxis(ControllerEvent):
    ...
    def __init__(self, type: str, which: int, axis: salient.sdl.joystick.ControllerAxis, value: int):
        super().__init__(type, which)
        self.axis = axis
    ...
    def from_sdl_event(cls, sdl_event: Any) -> ControllerAxis:
        return cls(
            ...
            salient.sdl.joystick.ControllerAxis(sdl_event.caxis.axis),
...
class ControllerButton(ControllerEvent):
    ...
    def __init__(self, type: str, which: int, button: salient.sdl.joystick.ControllerButton, pressed: bool):
        super().__init__(type, which)
        self.button = button
    ...
    def from_sdl_event(cls, sdl_event: Any) -> ControllerButton:
        ...
        return cls(
            ...
            salient.sdl.joystick.ControllerButton(sdl_event.cbutton.button),
...
class Undefined(Event):
    """This class is a place holder for SDL events without their own salient.event
    class.
    """
...
def _parse_event(sdl_event: Any) -> Event:
    """Convert a C SDL_Event* type into a salient Event sub-class."""
...
def wait(timeout: Optional[float] = None) -> Iterator[Any]:
    ...
    Returns the same iterator as a call to :any:`salient.event.get`.
    ...
    Example::
        ...
        context: salient.context.Context  # Context object initialized earlier.
        while True:  # Main game-loop.
            console: salient.Console  # Console used for rendering.
            ...
            for event in salient.event.wait():  # Sleeps until the next events exist.
                ...  # All events are handled at once before the next frame.

    See :any:`salient.event.get` examples for how different events are handled.
    """
...
class EventDispatch(Generic[T]):
    ...
    Example::
        ...
        MOVE_KEYS = {  # key_symbol: (x, y)
            # Arrow keys.
            salient.event.K_LEFT: (-1, 0),
            salient.event.K_RIGHT: (1, 0),
            salient.event.K_UP: (0, -1),
            salient.event.K_DOWN: (0, 1),
            salient.event.K_HOME: (-1, -1),
            salient.event.K_END: (-1, 1),
            salient.event.K_PAGEUP: (1, -1),
            salient.event.K_PAGEDOWN: (1, 1),
            salient.event.K_PERIOD: (0, 0),
            # Numpad keys.
            salient.event.K_KP_1: (-1, 1),
            salient.event.K_KP_2: (0, 1),
            salient.event.K_KP_3: (1, 1),
            salient.event.K_KP_4: (-1, 0),
            salient.event.K_KP_5: (0, 0),
            salient.event.K_KP_6: (1, 0),
            salient.event.K_KP_7: (-1, -1),
            salient.event.K_KP_8: (0, -1),
            salient.event.K_KP_9: (1, -1),
            salient.event.K_CLEAR: (0, 0),  # Numpad `clear` key.
            # Vi Keys.
            salient.event.K_h: (-1, 0),
            salient.event.K_j: (0, 1),
            salient.event.K_k: (0, -1),
            salient.event.K_l: (1, 0),
            salient.event.K_y: (-1, -1),
            salient.event.K_u: (1, -1),
            salient.event.K_b: (-1, 1),
            salient.event.K_n: (1, 1),
        }


        class State(salient.event.EventDispatch[None]):
            ...
            def ev_quit(self, event: salient.event.Quit) -> None:
                """The window close button was clicked or Alt+F$ was pressed."""
                print(event)
                self.cmd_quit()

            def ev_keydown(self, event: salient.event.KeyDown) -> None:
                """A key was pressed."""
                print(event)
                if event.sym in MOVE_KEYS:
                    # Send movement keys to the cmd_move method with parameters.
                    self.cmd_move(*MOVE_KEYS[event.sym])
                elif event.sym == salient.event.K_ESCAPE:
                    self.cmd_escape()

            def ev_mousebuttondown(self, event: salient.event.MouseButtonDown) -> None:
                """The window was clicked."""
                print(event)

            def ev_mousemotion(self, event: salient.event.MouseMotion) -> None:
        ...
        root_console = salient.console_init_root(80, 60)
        state = State()
        while True:
            salient.console_flush()
            for event in salient.event.wait():
    ...
    def ev_quit(self, event: salient.event.Quit) -> Optional[T]:
        """Called when the termination of the program is requested."""

    def ev_keydown(self, event: salient.event.KeyDown) -> Optional[T]:
        """Called when a keyboard key is pressed or repeated."""

    def ev_keyup(self, event: salient.event.KeyUp) -> Optional[T]:
        """Called when a keyboard key is released."""

    def ev_mousemotion(self, event: salient.event.MouseMotion) -> Optional[T]:
        """Called when the mouse is moved."""

    def ev_mousebuttondown(self, event: salient.event.MouseButtonDown) -> Optional[T]:
        """Called when a mouse button is pressed."""

    def ev_mousebuttonup(self, event: salient.event.MouseButtonUp) -> Optional[T]:
        """Called when a mouse button is released."""

    def ev_mousewheel(self, event: salient.event.MouseWheel) -> Optional[T]:
        """Called when the mouse wheel is scrolled."""

    def ev_textinput(self, event: salient.event.TextInput) -> Optional[T]:
        """Called to handle Unicode input."""

    def ev_windowshown(self, event: salient.event.WindowEvent) -> Optional[T]:
        """Called when the window is shown."""

    def ev_windowhidden(self, event: salient.event.WindowEvent) -> Optional[T]:
        """Called when the window is hidden."""

    def ev_windowexposed(self, event: salient.event.WindowEvent) -> Optional[T]:
        """Called when a window is exposed, and needs to be refreshed.

        This usually means a call to :any:`salient.console_flush` is necessary.
        """

    def ev_windowmoved(self, event: salient.event.WindowMoved) -> Optional[T]:
        """Called when the window is moved."""

    def ev_windowresized(self, event: salient.event.WindowResized) -> Optional[T]:
        """Called when the window is resized."""

    def ev_windowsizechanged(self, event: salient.event.WindowResized) -> Optional[T]:
        """Called when the system or user changes the size of the window."""

    def ev_windowminimized(self, event: salient.event.WindowEvent) -> Optional[T]:
        """Called when the window is minimized."""

    def ev_windowmaximized(self, event: salient.event.WindowEvent) -> Optional[T]:
        """Called when the window is maximized."""

    def ev_windowrestored(self, event: salient.event.WindowEvent) -> Optional[T]:
        """Called when the window is restored."""

    def ev_windowenter(self, event: salient.event.WindowEvent) -> Optional[T]:
        """Called when the window gains mouse focus."""

    def ev_windowleave(self, event: salient.event.WindowEvent) -> Optional[T]:
        """Called when the window loses mouse focus."""

    def ev_windowfocusgained(self, event: salient.event.WindowEvent) -> Optional[T]:
        """Called when the window gains keyboard focus."""

    def ev_windowfocuslost(self, event: salient.event.WindowEvent) -> Optional[T]:
        """Called when the window loses keyboard focus."""

    def ev_windowclose(self, event: salient.event.WindowEvent) -> Optional[T]:
        """Called when the window manager requests the window to be closed."""

    def ev_windowtakefocus(self, event: salient.event.WindowEvent) -> Optional[T]:
        pass

    def ev_windowhittest(self, event: salient.event.WindowEvent) -> Optional[T]:
        pass

    def ev_joyaxismotion(self, event: salient.event.JoystickAxis) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_joyballmotion(self, event: salient.event.JoystickBall) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_joyhatmotion(self, event: salient.event.JoystickHat) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_joybuttondown(self, event: salient.event.JoystickButton) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_joybuttonup(self, event: salient.event.JoystickButton) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_joydeviceadded(self, event: salient.event.JoystickDevice) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_joydeviceremoved(self, event: salient.event.JoystickDevice) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_controlleraxismotion(self, event: salient.event.ControllerAxis) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_controllerbuttondown(self, event: salient.event.ControllerButton) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_controllerbuttonup(self, event: salient.event.ControllerButton) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_controllerdeviceadded(self, event: salient.event.ControllerDevice) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_controllerdeviceremoved(self, event: salient.event.ControllerDevice) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """

    def ev_controllerdeviceremapped(self, event: salient.event.ControllerDevice) -> Optional[T]:
        """
        .. versionadded:: 13.8
        """
...
def add_watch(callback: _EventCallback) -> _EventCallback:
    ...
    Callbacks added as event watchers can later be removed with :any:`salient.event.remove_watch`.

    .. warning::
        How uncaught exceptions in a callback are handled is not currently defined by salient.
        They will likely be handled by :any:`sys.unraisablehook`.
        This may be later changed to pass the excpetion to a :any:`salient.event.get` or :any:`salient.event.wait` call.
    ...
    Example::
        ...
        @salient.event.add_watch
        def handle_events(event: salient.event.Event) -> None:
            if isinstance(event, salient.event.KeyDown):
                print(event)
...
def remove_watch(callback: Callable[[Event], None]) -> None:
    ...
    Args:
        callback (Callable[[Event], None]):
            A function which has been previously registered with :any:`salient.event.add_watch`.
...
def get_keyboard_state() -> NDArray[np.bool_]:
    ...
    Example::
        ...
        state = salient.event.get_keyboard_state()

        # Get a WASD movement vector:
        x = int(state[salient.event.Scancode.E]) - int(state[salient.event.Scancode.A])
        y = int(state[salient.event.Scancode.S]) - int(state[salient.event.Scancode.W])

        # Key with 'z' glyph is held:
        is_z_held = state[salient.event.KeySym.z.scancode]
...
class Scancode(enum.IntEnum):
    ...
    def __repr__(self) -> str:
        ...
        return f"salient.event.{self.__class__.__name__}.{self.name}"
...
class KeySym(enum.IntEnum):
    ...
    def label(self) -> str:
        ...
        Example::
            ...
            >>> salient.event.KeySym.F1.label
            'F1'
            >>> salient.event.KeySym.BACKSPACE.label
            'Backspace'
        """
