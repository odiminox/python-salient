...
def get_version() -> str:
    """Get the current version from a git tag, or by reading salient/version.py"""
    if (SETUP_DIR / ".git").exists():
        ...
        # update salient/version.py
        (SETUP_DIR / "salient/version.py").write_text(f'__version__ = "{version}"\n', encoding="utf-8")
        return version
    else:  # Not a Git repository.
        try:
            match = re.match(r'__version__ = "(\S+)"', (SETUP_DIR / "salient/version.py").read_text(encoding="utf-8"))
            assert match
...
def get_package_data() -> List[str]:
    """get data files which will be included in the main salient/ directory"""
    BITSIZE, _ = platform.architecture()
    files = [
        "py.typed",
        "lib/SALIENT-CREDITS.txt",
        "lib/SALIENT-LICENSE.txt",
        "lib/README-SDL.txt",
    ]
...
if not (SETUP_DIR / "libsalient/src").exists():
    print("Libsalient submodule is uninitialized.")
...
setup(
    ...
    project_urls={
        "Documentation": "https://python-salient.readthedocs.io",
        "Changelog": "https://github.com/salient/python-salient/blob/main/CHANGELOG.md",
        "Source": "https://github.com/salient/python-salient",
        "Tracker": "https://github.com/salient/python-salient/issues",
        "Forum": "https://github.com/salient/python-salient/discussions",
    },
    py_modules=["salientpy"],
    packages=["salient", "salient.sdl", "salient.__pyinstaller"],
    package_data={"salient": get_package_data()},
    python_requires=">=3.7",
    setup_requires=[
        *pytest_runner,
        "cffi>=1.15",
    ...
    cffi_modules=["build_salient.py:ffi"],
    ...
    keywords="roguelike cffi Unicode libtcod field-of-view pathfinding salient",
